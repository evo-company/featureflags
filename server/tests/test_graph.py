...
from featureflags.server.graph.graph import GRAPH, pull, _is_uuid
from featureflags.server.graph.proto import populate
from featureflags.server.schema import Action
...
async def test_root_flag_invalid(sa, hiku_engine):
    query = build([
        Q.flag(id='invalid-uuid')[
            Q.id,
        ],
    ])
    result = await pull(hiku_engine, query, sa=sa,
                        session=auth.TestSession(uuid4()))
    assert denormalize(GRAPH, result) == {'flag': None}


@pytest.mark.asyncio
@pytest.mark.parametrize('enabled, overridden', [
    (None, False),
    (True, True),
    (False, True),
])
...
@pytest.mark.parametrize('state, value', [
    ...
    (auth.ExpiredAccessToken(uuid4(), 'secret', 'session_key',
