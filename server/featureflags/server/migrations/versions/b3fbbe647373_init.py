...
def upgrade():
    ...
    op.create_index('auth_session_expiration_time', 'auth_session', ['expiration_time'], unique=False)
    op.create_index('auth_session_user_creation_time', 'auth_session', ['user', 'creation_time'], unique=False)
    op.create_table('local_id_map',
    ...
    op.create_index('flag_project_name_idx', 'flag', ['project', 'name'], unique=False)
    op.create_table('variable',
    sa.Column('id', postgresql.UUID(as_uuid=True), nullable=False),
    sa.Column('name', sa.String(), nullable=False),
    sa.Column('type', sa.Enum('STRING', 'NUMBER', 'TIMESTAMP', 'SET', name='type'), nullable=False),
    sa.Column('project', postgresql.UUID(as_uuid=True), nullable=False),
    sa.ForeignKeyConstraint(['project'], ['project.id'], ),
    sa.PrimaryKeyConstraint('id'),
    sa.UniqueConstraint('project', 'name')
    )
    op.create_index('variable_project_name_idx', 'variable', ['project', 'name'], unique=False)
    op.create_table('check',
    sa.Column('id', postgresql.UUID(as_uuid=True), nullable=False),
    sa.Column('operator', sa.Enum('EQUAL', 'LESS_THAN', 'LESS_OR_EQUAL', 'GREATER_THAN', 'GREATER_OR_EQUAL', 'CONTAINS', 'PERCENT', 'REGEXP', 'WILDCARD', 'SUBSET', 'SUPERSET', name='operator'), nullable=False),
    ...
    sa.ForeignKeyConstraint(['variable'], ['variable.id'], ),
    sa.PrimaryKeyConstraint('id')
    )
    op.create_table('condition',
    sa.Column('id', postgresql.UUID(as_uuid=True), nullable=False),
    sa.Column('flag', postgresql.UUID(as_uuid=True), nullable=False),
    sa.Column('checks', postgresql.ARRAY(postgresql.UUID(as_uuid=True), as_tuple=True), nullable=True),
    sa.ForeignKeyConstraint(['flag'], ['flag.id'], ),
    sa.PrimaryKeyConstraint('id')
    )
