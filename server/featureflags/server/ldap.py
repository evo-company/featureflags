...
def _check_credentials(server, dn, password, receive_timeout):
    with Connection(server, user=dn, password=password,
                    receive_timeout=receive_timeout) as conn:
        is_bound = conn.bind()
...
class DummyLDAP:
    ...
    async def check_credentials(self, user, password, *,
                                connect_timeout=5, receive_timeout=5):
        return self._bound
...
class LDAP:
    ...
    async def check_credentials(self, user, password, *,
        ...
        result = await loop.run_in_executor(None, _check_credentials,
                                            server, dn, password,
                                            receive_timeout)
        return result
